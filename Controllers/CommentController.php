...
class CommentController {
    ...
    public function updateComment() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($updated) {
                // Fetch the updated comment using readSingle
                $this->comment->readSingle();
    
                echo json_encode([
                    "success" => true,
                    "message" => "Comment updated successfully!",
                    "updated_content" => htmlspecialchars($this->comment->content),
                    "updated_at" => htmlspecialchars($this->comment->created_at),
                    "comment_id" => $id,
